...
type Article = { no:string; tag:string; title:string; intro:string; date:string; sections:{label:string;title:string;text:string}[] };
...
export default function ArticlePage({ article }: { article: Article }) {
  return <main className="article-shell">
    ...
    <section className="article-table"><h2>method reference</h2><div className="table-head"><span>stage</span><span>signal</span><span>reference</span><span>output</span></div>{[['capture','EEG + event marker','participant clock','raw stream'],['align','stimulus onset','shared timeline','epochs'],['evaluate','ERP / features','quality transition','NeuroQoE']].map(row=><div className="table-row" key={row[0]}>{row.map(cell=><span key={cell}>{cell}</span>)}</div>)}</section>
